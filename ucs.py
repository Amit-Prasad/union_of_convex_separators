...
def add_hyperplane(W, S, data_x, data_y, first, pdf, visualise = False):
    w, not_in_the_hull, separator_index, x = learn_init_hyperplane(W, S, data_x, data_y, first, pdf, visualise)
...
def train_ucs(data_x, data_y, n_hyp, out_file, visualise = False):
    W = []
...
def main():
    for a in range(2, 3):
        ...
        first = True
        for i in range(0, 10):
            W, added_hyperplanes = add_hyperplane(W, S, data_x, data_y, first,  pdf, visualise = True)
            for j in range(0, 10):
                pdf.text(20, 436, 'E step')
                pdf.add_page()
                S = refresh_memberships(W, data_x)
                draw_memberships(W, S, data_x, pdf)
                pdf.add_page()
                W = learn_hyperplanes(W, S, data_x, data_y, 0.001, 10*i+j, pdf, added_hyperplanes,  visualise = True)
                S = refresh_memberships(W, data_x)
                draw_memberships(W, S, data_x, pdf)
                pdf.add_page()
            first = False
        pdf.output('result' + str(a) + '.pdf')
        for i in range(0, len(W)):
            print(W[i].shape)
