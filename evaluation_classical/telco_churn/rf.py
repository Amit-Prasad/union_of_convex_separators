...
def rf_run(data_name):
    ...
    parameters = {'n_estimators' : [5], 'min_samples_split' : [10], 'max_features' : ['sqrt'], 'max_samples' : [0.5]} #best
    ...
    clf = GridSearchCV(random_forest, parameters, n_jobs = -23, scoring='balanced_accuracy')
    clf.fit(data_x, data_y)
    output_dict['best_params'] = clf.best_params_
    clf = clf.best_estimator_
    scores = cross_val_score(clf, data_x, data_y, cv=5, n_jobs = -23, scoring = 'balanced_accuracy')
