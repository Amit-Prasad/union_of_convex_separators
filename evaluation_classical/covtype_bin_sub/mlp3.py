...
def mlp3_run(data_name):
    output_dict = {}
    output_dict['dataset'] = data_name
    output_dict['algo'] = 'mlp3'
    ...
    for i in range(10, 250, 10):
        for j in range(10, 250, 10):
            for k in range(10, 250, 10):
                hidden_comb.append((i,j,k,))

    parameters = {'hidden_layer_sizes': hidden_comb}
    #best {'hidden_layer_sizes': (200, 220, 210)}
    nn = MLPClassifier(random_state=1, max_iter=10000)
    clf = GridSearchCV(nn, parameters, n_jobs=-1, scoring='balanced_accuracy')
